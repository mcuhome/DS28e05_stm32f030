...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  for (pinpos = 0x00; pinpos < 0x10; pinpos++)//循环16次
  {
    pos = ((uint32_t)0x01) << pinpos;

    /* Get the port pins position */
    currentpin = (GPIO_InitStruct->GPIO_Pin) & pos;//假设P07=0x0080
...
void __inline GPIO_Init_in(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct)
{
  uint32_t pinpos = 0x00, pos = 0x00 , currentpin = 0x00;
...
//	  pos=0x80;
//	  pinpos=0x07;
//	printf("pinpos=%x,pos=%x",pinpos,pos);
...
/**
  ...
  */
uint8_t __inline GPIO_ReadInputDataBit(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
